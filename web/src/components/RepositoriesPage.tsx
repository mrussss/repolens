...
export const RepositoriesPage: React.FC<Props> = ({ onSelectRepoForDiagnosis }) => {
  ...
  const loadRepos = async () => {
    try {
      ...
      setError(err.message || 'Failed loading repositories');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateRepo = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Failed adding repository');
    }
  };
  ...
  const handleTriggerIndex = async (repoId: string, ref: string) => {
    ...
    try {
      ...
      setError(err.message || 'Failed initiating repository indexing');
    } finally {
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <div>
          <h1 style={{ fontSize: '1.75rem', fontWeight: 700, color: 'var(--text-bright)' }}>Repositories</h1>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>
            Manage indexed Git repositories and materialization snapshots.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="card" style={{ textAlign: 'center', padding: '3rem 1rem' }}>
          ...
          <h3 style={{ fontSize: '1.1rem', color: 'var(--text-bright)', marginBottom: '0.5rem' }}>No Repositories Yet</h3>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem', marginBottom: '1.5rem' }}>
            Add a Git repository URL (GitHub/GitLab) or click <strong>Try Demo</strong> in Setup to explore sample codebases.
          </p>
          <button className="btn btn-primary" onClick={() => setShowAddModal(true)}>
            <Plus size={16} /> Add First Repository
          </button>
        </div>
      ) : (
      ...
      {showAddModal && (
        <div style={{
          ...
          <div className="card" style={{ width: 500, maxWidth: '90%' }}>
            <h2 style={{ fontSize: '1.25rem', color: 'var(--text-bright)', marginBottom: '1rem' }}>Add Git Repository</h2>
            <form onSubmit={handleCreateRepo}>
              ...
              <div className="form-group">
                <label className="form-label">Git HTTPS URL</label>
                <input
                  type="url"
                  className="input-field"
                  value={gitURL}
                  ...
                  required
                />
                <div className="form-hint">HTTPS only; protected against SSRF and private IPs.</div>
              </div>

              <div className="form-group">
                <label className="form-label">Default Branch / Ref</label>
                <input
                  type="text"
                  className="input-field"
                  value={defaultRef}
