...
export const SetupPage: React.FC<Props> = ({ onDemoStarted, onConfigSaved }) => {
  ...
  const [testResult, setTestResult] = useState<Awaited<ReturnType<typeof api.testProviderConnection>> | null>(null); const [saveSuccess, setSaveSuccess] = useState(false); const [error, setError] = useState<string | null>(null);
  ...
  const handleTest = async () => { if (authMode === 'bearer' && !apiKey) { setError('Bearer 鉴权模式需要填写 API Key'); return; } setTesting(true); setError(null); setTestResult(null); try { setTestResult(await api.testProviderConnection({ base_url: baseURL, model, api_key: apiKey, auth_mode: authMode })); } catch (err: any) { setTestResult({ success: false, latency_ms: 0, code: err.code, message: err.message }); } finally { setTesting(false); } };
  ...
  const testAlert = testResult ? getProviderTestAlert(testResult) : null;
